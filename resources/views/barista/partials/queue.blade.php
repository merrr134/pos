{{--
    Fragment antrian Barista (FR-007) — mirror kitchen/partials/queue dengan AKSEN AMBER
    (pembeda station sesuai SRS; class literal hardcode agar purge-safe).
    Dipakai dua arah: include awal di barista/index.blade.php (#barista-queue) dan
    dikembalikan endpoint barista.queue-status untuk partial refresh.
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-amber-800">
                        {{ $next->table->name }} memesan {{ $next->items->sum('qty') }} item minuman — prioritaskan order ini.
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500">
                        {{ $next->table->name }} memesan {{ $next->items->sum('qty') }} item minuman.
                    </p>
                </div>
            </div>
            <a href="#order-{{ $next->id }}"
               class="inline-flex items-center gap-2 whitespace-nowrap rounded-lg bg-amber-600 px-4 py-2 text-sm font-semibold text-white hover:bg-amber-700">
                Lihat Pesanan
            </a>
        </div>
    @endif
...
<div class="mb-3 flex items-center gap-2">
    ...
    <span class="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-800">{{ $unprinted->count() }}</span>
</div>
...
<div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <div id="order-{{ $order->id }}"
            ...
            <div class="flex items-start justify-between p-4 pb-0">
                <div class="flex items-center gap-1.5">
                    <span class="inline-flex items-center rounded-md bg-amber-100 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-amber-800">
                        {{ $order->order_number }}
                    </span>
                    @if ($isVip)
            ...
            <div class="flex-1 space-y-2 p-4">
                @foreach ($order->items as $item)
                    <div class="flex items-center gap-2 text-sm">
                        <span class="w-8 shrink-0 font-bold text-amber-700">{{ $item->qty }}x</span>
            ...
            <div class="border-t border-slate-100 p-3">
                <a href="{{ route('barista.checker', $order) }}" target="_blank"
                   class="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 hover:border-amber-600 hover:text-amber-700">
    ...
    <div class="flex min-h-[220px] flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-slate-200 bg-white/60 p-6 text-center">
        <span class="flex h-12 w-12 animate-pulse items-center justify-center rounded-full bg-amber-50 text-amber-300">
