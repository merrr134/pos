...
<div x-data="stationQueue({ lastId: {{ $lastItemId }}, signature: @js($signature), unprintedCount: {{ $unprinted->count() }} })">
    ...
    <div class="hero-header mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="font-display text-2xl font-bold">Station Barista</h2>
            <p class="mt-1 text-sm text-white/80">Antrian item minuman dari order aktif. Layar diperbarui otomatis.</p>
        </div>
        <div class="flex items-center gap-3">
            <span class="inline-flex items-center gap-1.5 rounded-lg bg-white/15 px-3 py-2 text-sm font-semibold">
                <x-lucide-cup-soda class="h-4 w-4" />
            ...
            <button type="button" @click="toggleSound()"
                    class="inline-flex items-center gap-1.5 rounded-lg bg-white px-3 py-2 text-sm font-semibold text-amber-700 hover:bg-white/90">
    ...
    <div id="barista-queue">
        @include('barista.partials.queue')
    </div>
</div>
@endsection
...
@push('scripts')
    ...
    @include('station.partials.queue-script', [
        'pollUrl'     => route('barista.queue-status'),
        'containerId' => 'barista-queue',
    ])
@endpush
